...
export const projects = [
  {
    ...
    impact: "Offline-first Korean course — real scenarios, not isolated drills",
    shortDescription:
      "An offline-first language-learning platform that prepares learners for real conversations. The first course teaches Korean to English- and Vietnamese-speaking learners (A0 → practical A1, plus selected A2 travel and social scenarios).",
    description:
      "Vernora is built around scenario-based lessons — order food, ask directions, check into a hotel — with production-heavy exercises from day one. Content works fully offline after download; progress syncs safely later. AI supports learning with deterministic fallbacks, never as a hard dependency.",
    overview:
      "Most language apps optimize for streaks and isolated drills. Vernora optimizes for what you can actually do in the world: after completing a level, you should be able to perform the real-world scenarios tied to that level.\n\nThe first course targets Korean for English- and Vietnamese-speaking learners, spanning A0 through practical A1 with selected A2 travel and social situations. Lessons emphasize natural speech — politeness levels, slang, texting conventions, and cultural context — not textbook phrases alone.\n\nA personal error memory tracks your recurring mistakes and surfaces them in review. The free core curriculum stays usable without AI; when AI is involved, it is a supporting layer with deterministic fallbacks, not a single point of failure.\n\nThe monorepo is organized for scale: a React Native (Expo) mobile learner app with SQLite and TanStack Query, a Spring Boot modular monolith API with PostgreSQL and Flyway, a Next.js admin app for content, shared packages for schema and design tokens, and Docker-based infrastructure with GitHub Actions. Speech recognition is planned via whisper.cpp.\n\nThe project is in pre-development — currently building the product foundation (Phase 1 of the roadmap). A landing page is live at vernora.vercel.app while the learner app and backend are under active design and implementation.",
    technologies: [
      "TypeScript",
      "React Native",
      "Expo",
    ...
    techStack: [
      {
        ...
        role: "Offline-first learner app with downloaded content packs; TanStack Query for sync when connectivity returns.",
      },
      {
        layer: "Backend",
        technology: "Java 21 + Spring Boot",
        role: "Modular monolith API for progress, content delivery, and sync — PostgreSQL with Flyway migrations.",
      },
      {
        layer: "Admin",
        technology: "Next.js + TypeScript + Tailwind",
        role: "Content administration for scenarios, lessons, and curriculum authoring.",
      },
      {
        layer: "Platform",
        technology: "Supabase + Docker + GitHub Actions",
        role: "Postgres, auth, and storage in prod; Compose for local dev; CI for the monorepo.",
      },
      {
        layer: "Shared packages",
        technology: "Content schema + API contract",
        role: "Single source of truth for lesson structure and types across mobile, admin, and API.",
      },
      {
        layer: "Speech (planned)",
        technology: "whisper.cpp (C++20)",
        role: "On-device speech recognition for production-heavy speaking exercises without cloud dependency.",
      },
      {
        layer: "Product design",
        technology: "Scenario-based curriculum + error memory",
        role: "Lessons map to real situations; the app remembers your recurring mistakes for targeted review.",
      },
    ],
    features: [
      "Scenario-based lessons (order food, ask directions, check in) instead of isolated drills",
      "Production-heavy exercises — learners speak and write from lesson one",
      "Fully offline after content download; progress syncs safely when back online",
      "Natural speech: politeness levels, slang, texting conventions, and cultural context",
      "Personal error memory that reviews your recurring mistakes",
      "Free core curriculum; AI as a supporting feature with deterministic fallbacks",
      "First course: Korean for English- and Vietnamese-speaking learners (A0 → A1 + selected A2)",
    ],
    challenges: [
      "Delivering a credible offline-first experience without stale content or broken sync",
      "Balancing AI assistance with deterministic fallbacks so learning never depends on a model",
      "Designing scenario progressions that map cleanly to real-world speaking ability",
    ],
    solutions: [
      "SQLite-backed content packs on device with safe background sync through the Spring Boot API",
      "AI kept optional — core lesson flow and grading paths work without cloud inference",
      "Level outcomes tied explicitly to performable real-world scenarios, documented in the product spec and ADRs",
    ],
    demoUrl: "https://vernora.vercel.app/",
    githubUrl: "https://github.com/radleyle/vernora",
    status: "In Progress",
    timeline: "Ongoing",
  },
  {
    id: "kitchenlab",
    title: "KitchenLab",
    label: "Full-Stack AI Agent",
    impact: "Live at kitchen-lab-tau.vercel.app — LLM phrases, Python decides",
    shortDescription:
      "Cook better through science. An AI cooking agent that teaches, generates and adapts recipes, diagnoses failures, suggests substitutions, and runs small kitchen experiments — grounded in cited food-science passages and deterministic calculators, never LLM guesswork.",
    description:
    ...
    techStack: [
      {
        ...
        role: "Ask, Recipes, Lab, Calculators, and My Kitchen pages — deployed on Vercel; port 3001 in local Compose.",
      },
      {
        layer: "Backend",
        technology: "Python 3.12 + FastAPI + Uvicorn",
        role: "Agent router, RAG, calculators, and REST surface with interactive `/docs`; Render (Docker) in prod, port 8000 locally.",
      },
      {
        layer: "Database",
        technology: "PostgreSQL 16 + pgvector",
        role: "Users, recipes, experiments, notebook, and vector embeddings for semantic passage retrieval — Neon in prod.",
      },
      {
        layer: "ORM & migrations",
        technology: "SQLAlchemy 2 + Alembic",
        role: "Versioned schema for the full domain model; idempotent seed scripts for safety, knowledge, symptoms, and techniques.",
      },
      {
        layer: "Auth & profile",
        technology: "JWT (PyJWT) + bcrypt",
        role: "Stateless login with hashed passwords; optional kitchen profile sends a Bearer token for personalized cook/adapt/substitute.",
      },
      {
        layer: "LLM & embeddings",
        technology: "OpenAI (`gpt-4o-mini`, `text-embedding-3-small`)",
        role: "Phrasing and semantic search over curated passages — not arithmetic, conversions, or safety limits.",
      },
      {
        layer: "Knowledge & safety",
        technology: "Cited RAG + USDA tables + calculators",
        role: "Claim-level citations with scope and source metadata; internal temps and volume→grams enforced in Python after generation.",
      },
      {
        layer: "Agent modes",
        technology: "learn · cook · adapt · diagnose · substitute · experiment",
        role: "Intent router dispatches to mode handlers — from technique Q&A and recipe generation to failure diagnosis and controlled trials.",
      },
      {
        layer: "Eval & CI",
        technology: "pytest + GitHub Actions",
        role: "Deterministic eval scenarios guard trust contracts on every push; optional live LLM evals for integration checks.",
      },
      {
        layer: "Media",
        technology: "Unsplash API + local / S3 storage",
        role: "Attributed recipe cover photos in prod; experiment and notebook attachments via a unified storage backend.",
      },
      {
        layer: "Infrastructure",
        technology: "Docker Compose · Terraform (AWS reference)",
        role: "Identical local stack via Compose; live prod on Vercel + Render + Neon; optional infra/ maps roles to ECS, RDS, S3, and ALB.",
      },
    ],
    features: [
      "learn — grounded Q&A plus a techniques library when a named method matches",
      "cook — science-annotated recipe generation with USDA safety floors applied in Python",
      "adapt — paste any recipe; Python standardizes measures and adds food-science annotations",
      "diagnose — symptom taxonomy, follow-ups, then evidence-ranked causes with cited fixes",
      "substitute — function-aware swaps (moisture vs binder vs leavening roles)",
      "experiment — structured trial design with observations, photos, and notebook conclusions",
      "Calculators page for brine, scaling, baker's %, and volume→grams — pure Python math",
      "Personal cookbook and lab notebook; kitchen profile personalizes output when signed in",
    ],
    challenges: [
      "Keeping the trust split intact so the LLM never overrides USDA temps, conversions, or allergen rules",
      "Substitution logic that respects what an ingredient does in a recipe, not just what it's called",
      "Encoding those guarantees as automated eval scenarios instead of manual spot-checks",
    ],
    solutions: [
      "Post-generation Python enforcement for safety temps, oven dial offsets, and unit math before anything reaches the user",
      "Diagnosis cause ranking runs as deterministic scoring; the LLM only classifies free-text as supports, contradicts, or neutral",
      "pytest eval harness with scenario catalog in CI; curated RAG passages carry claim-level citations with confidence metadata",
    ],
    demoUrl: "https://kitchen-lab-tau.vercel.app/",
    githubUrl: "https://github.com/radleyle/kitchen-lab",
    status: "In Progress",
    timeline: "Ongoing",
  },
  {
