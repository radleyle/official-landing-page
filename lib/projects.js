...
export const projects = [
  {
    ...
    impact: "LLM personalizes — USDA tables, calculators, and citations decide",
    shortDescription:
    ...
    description:
      "KitchenLab is an AI cooking agent built on a strict trust split: the LLM phrases and personalizes responses, but it never invents temperatures, gram conversions, or safety facts. Those come from deterministic calculators, USDA tables, and curated passages with claim-level citations.",
    overview:
      "Most cooking assistants guess. KitchenLab separates evidence from explanation. A cited food-science knowledge base and deterministic calculators produce the facts — temperatures, ratios, timing, food-safety limits — and the LLM agent teaches, generates recipes, diagnoses failures, suggests substitutions, and designs kitchen experiments from that grounded output.\n\nThe stack runs locally via Docker Compose: a Next.js + TypeScript frontend (port 3001), a Python + FastAPI backend with the agent, RAG, and calculators (port 8000), and PostgreSQL 16 with pgvector for relational data and semantic search. OpenAI powers phrasing and embeddings only — not facts or math. Production target is ECS Fargate, RDS, and S3; CI already runs pytest evals on GitHub Actions.\n\nSix agent modes cover the full workflow: learn (grounded Q&A), cook (science-annotated recipes with USDA safety enforced in Python), adapt (standardize measures in code), diagnose (symptom taxonomy → ranked causes), substitute (function-aware swaps), and experiment (controlled trials with observations and photos). An optional kitchen profile personalizes cook, adapt, and substitute when authenticated.",
    technologies: [
      "Next.js",
      "TypeScript",
      "FastAPI",
    ...
    techStack: [
      {
        ...
        role: "Typed React UI for all agent modes; runs on port 3001 locally.",
      },
      {
        layer: "Backend",
        technology: "Python 3.12 + FastAPI + Uvicorn",
        role: "HTTP API, agent orchestration, RAG retrieval, and deterministic calculators; auto `/docs` playground on port 8000.",
      },
      {
        layer: "Database",
        technology: "PostgreSQL 16 + pgvector",
        role: "Relational storage for users, recipes, experiments, and notebook data; vector search for semantic RAG.",
      },
      {
        layer: "ORM & migrations",
        technology: "SQLAlchemy 2 + Alembic",
        role: "Python models with versioned schema migrations — database changes tracked like code.",
      },
      {
        layer: "Validation & config",
        technology: "Pydantic + pydantic-settings",
        role: "Request/response shapes and environment configuration with strict typing.",
      },
      {
        layer: "Auth",
        technology: "JWT (PyJWT) + bcrypt",
        role: "Stateless login; passwords stored as hashes only. Optional kitchen profile personalizes cook/adapt/substitute.",
      },
      {
        layer: "LLM & embeddings",
        technology: "OpenAI (`gpt-4o-mini`, `text-embedding-3-small`)",
        role: "Phrasing and semantic search only — never the source of temperatures, conversions, or safety facts.",
      },
      {
        layer: "Knowledge & safety",
        technology: "Cited RAG + USDA tables + calculators",
        role: "Claim-level citations from curated food-science passages; USDA safety floors and unit math enforced in Python.",
      },
      {
        layer: "Agent modes",
        technology: "learn · cook · adapt · diagnose · substitute · experiment",
        role: "Six modes: Q&A, recipe generation, recipe adaptation, failure diagnosis, function-aware substitutions, and controlled kitchen experiments.",
      },
      {
        layer: "Infrastructure",
        technology: "Docker Compose + GitHub Actions",
        role: "Identical local stack on every machine; pytest unit and eval scenarios run in CI on every push and PR. Production target: ECS Fargate, RDS, S3.",
      },
      {
        layer: "Media",
        technology: "Local filesystem (`MEDIA_ROOT`)",
        role: "Experiment and notebook photo attachments today; S3-shaped paths ready for production drop-in.",
      },
    ],
    features: [
      "learn — grounded Q&A with a technique library when a named technique matches",
      "cook — science-annotated recipe generation; USDA safety floor enforced in Python, not the LLM",
      "adapt — paste a recipe → Python standardizes measures and annotates with food-science notes",
      "diagnose — symptom taxonomy → follow-up questions → evidence-ranked causes with cited fixes",
      "substitute — function-aware swaps (egg-as-moisture ≠ egg-as-binder)",
      "experiment — designs controlled trials; log observations, photos, and conclusions",
      "Optional kitchen profile (oven offset, elevation, equipment, allergies) personalizes responses when authenticated",
    ],
    challenges: [
      "Enforcing the trust split so the LLM never invents temperatures, gram conversions, or food-safety facts",
      "Function-aware substitutions that respect ingredient role in a recipe, not just name matching",
      "Designing controlled kitchen experiments that are scientific enough to be useful but simple enough to run at home",
    ],
    solutions: [
      "Route all numeric, conversion, and safety outputs through deterministic calculators and USDA tables before any LLM response",
      "RAG over curated food-science passages with claim-level citations; OpenAI used for phrasing and embeddings only",
      "Six typed agent modes with Pydantic validation; pytest eval scenarios and GitHub Actions CI guard deterministic behavior",
    ],
    demoUrl: "",
    githubUrl: "",
    status: "In Progress",
